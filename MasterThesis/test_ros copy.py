...
import threading
import omni.kit.app
print(f'print omni.kit.app', omni.kit.app.__file__)
...
class TimedNode(Node):
    def __init__(self, runtime):
        ...
        # 创建定时器，每秒触发一次
        self.timer = self.create_timer(1.0, self.check_runtime)

    def check_runtime(self):
        elapsed_time = time.time() - self.start_time
        self.get_logger().info(f"Running for {elapsed_time:.2f} seconds")
        if elapsed_time >= self.runtime:
            self.get_logger().info('Shutting down node after timeout.')
            rclpy.shutdown()
